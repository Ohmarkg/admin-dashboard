...
| POST | `/revoke` | `{ uids: string[] }` (1–200, deduped server-side) | for each uid: pop the **last** `instagramLogs` timestamp and subtract the event's `signInPoints` from `points` (floored at 0), merge-set to BOTH log paths in one atomic batch | The undo for an award given to the wrong member. Removes **one** award per call, not the member's whole history, so weekly awards stay individually correctable. Web-only — the mobile `addInstagramPoints` callable has no inverse — but writes the same doc shape, so the two coexist. Resolves the event **read-only** (`findInstagramEvent`), using the same lexicographically-smallest selection as `/award` so a revoke always edits the doc the award wrote to; no event exists → `404 instagram_event_not_found` (removing points must never create an event). When the last award is removed the two log docs are **deleted** rather than left as an empty 0-award row — guarded, so a log that carries real `signInTime`/`signOutTime` is emptied instead of deleted. uids with no log, or an already-empty history, are skipped and reported. Response: `{ ok: true, eventId, revoked, notAwarded, pointsPerAward }` |
